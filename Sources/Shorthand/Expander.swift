...
final class Expander {
    ...
    /// Community-standard markers (nspasteboard.org) telling clipboard
    /// managers to ignore a programmatic write. Without these, every
    /// expansion shows up in clipboard history apps twice: once for the
    /// snippet paste and once for the clipboard restore.
    ...
    /// This is a deliberate user copy, so it is NOT marked transient:
    /// clipboard managers should keep it like any other copy.
    func copyToPasteboard(_ snippet: Snippet) {
        let rendered = render(snippet)
        writePasteboard(rendered, transient: false)
    }
    ...
    private func pasteRendered(_ snippet: Snippet) {
        ...
        writePasteboard(rendered, transient: true)
    ...
    private func writePasteboard(_ rendered: Rendered, transient: Bool) {
        let pasteboard = NSPasteboard.general
        pasteboard.clearContents()
        let item = NSPasteboardItem()
        if transient {
            for marker in Self.transientMarkers {
    ...
    private func restore(_ pasteboard: NSPasteboard, items: [[NSPasteboard.PasteboardType: Data]]) {
        ...
        let restored = items.map { byType -> NSPasteboardItem in
            ...
            // Managers already saw this content when the user first copied
            // it; marking the restore transient stops them capturing it twice.
            for marker in Self.transientMarkers {
                item.setData(Data(), forType: marker)
            }
            return item
